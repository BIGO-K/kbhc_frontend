...
export const useMemberInformation = () => {
	...
	return {
		...
		get: async () => {
			...
			expireStore.set('memberInfo', memberInfo, { expired_at: monsDate().add(10, 'minute').toDate() });

			return memberInfo;
		},
		/** 회원정보 수정 */
		update: async (form: PersonalInformationForm) => {
		...
		/** 재인증(재로그인) */
		reverify: async (password: string) => {
		...
		/** 소셜로그인 재인증(재로그인) */
		reverifyByThirdParty: async (platform: string, thirdPartyAccessToken: string) => {
		...
		/** 현재 비밀번호 일치 여부 확인 */
		checkCurrentPassword: async (password: string) => {
		...
		/** 회원 탈퇴 처리 */
		withdraw: async (reasonCode: number, reasonDetail: string) => {
		...
		connectSocial: async (platform: ThirdPartyPlatforms, token: string) => {
			await useBackend().command(`v1/3rd-party-authentications/${platform}/${token}/connect`, { method: 'post' });
		},
		disConnectSocial: async (platform: ThirdPartyPlatforms) => {
