...
export const useGoodsZoomLoaderImage = () => {
	const animate = useAnimation();
	const cloneImageInterval = ref<Nullable<number>>(null);

	const goodsZoomLoad = (iElement?: HTMLElement, targetElement?: Nullable<HTMLElement>) => {
		if (iElement == null || targetElement == null) return;
		if (document.querySelector('.m_product-clone')) return;

		const wrapElement = document.createElement('div');
		wrapElement.classList.add('m_product-clone');

		iElement.removeAttribute('data-lazyload');
		iElement.setAttribute('class', 'mm_bg-cover image_product S=loaded');// XXX classList.add로 안됨?
		iElement.style.width = '100%';
		iElement.style.paddingTop = '100%';

		wrapElement.appendChild(iElement);
		wrapElement.style.position = 'fixed';
		wrapElement.style.background = 'rgba(#fff, 0.7)';
		wrapElement.style.zIndex = '9999';
	...
	const clearCloneImageInterval = () => {
		if (!cloneImageInterval.value) return;
	...
	return {
		...
		removeCloneImage: () => {
			...
			if (!cloneImage || cloneImageInterval.value) return clearCloneImageInterval();
