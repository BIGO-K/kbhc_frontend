...
export const useFindAuth = () => {
	const FIND_TYPE: Record<string, LoginInfoFindType> = {
		phone: '_phone',
		email: '_email',
		_phone: 'phone',
		_email: 'email',
	...
	return {
		...
		passwordFindData,
		/** 아이디 찾기 */
		findId: async (form: IdFindForm) => {
			const { find_type, ...findForm } = form;
			const { find_results, user_name, access_token, expire_at } = await useBackend().command<ExpireToken & IdFindResult>('v1/members/find-login-id', {
				method: 'post',
				data: {
		...
		/** 비밀번호 찾기 */
		findPassword: async (form: PasswordFindForm) => {
		...
		/** 비밀번호 변경 인증번호 전송 */
		sendPasswordFindVerificationCode: async (form: PasswordVerificationCodeForm) => {
		...
		/** 비밀번호 변경 인증번호 확인 */
		verifyPasswordCode: async (code: number) => {
			if (!passwordFindData.value.access_token) {
				throw new Error('세션이 만료되었습니다.');
			}
