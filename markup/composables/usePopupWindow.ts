...
export const usePopupWindow = (
	...
	const popupWindow = ref<Nullable<Window>>(null);
	const popupWindowClosed = ref(false);

	const popupWindowIntervalHandle = useIntervalFn(() => {
		if (popupWindow.value?.closed === true) popupWindowClosed.value = true;
	}, 1000);
	...
	const popupOpen = (url: string) => {
		...
		if (!popupWindow.value) return Promise.reject(new Error('팝업 차단 해제 후 다시 시도해주세요.'));
