import { ref, MaybeRef, isRef, unref, computed, watch } from 'vue';
...
export const usePasswordValidator = (
	...
	const doValidate = () => {
        ...
        /** 조합 확인 */
		let comboCount = 0;
		if (vPassword.search(/[0-9]/g) > -1) ++comboCount;
		if (vPassword.search(/[a-z]/g) > -1) ++comboCount;
		if (vPassword.search(/[A-Z]/g) > -1) ++comboCount;
		if (vPassword.search(/[{}[\]()<>/\\|,.?;:'"`~!@#$%^&*-_=+]/gi) > -1) ++comboCount;
		...
		if (/([a-zA-Z0-9{}[\]()<>/\\|,.?;:'"`~!@#$%^&*-_=+])\1{3,}/g.test(vPassword)) {
			formValid.value.state = 'danger';
        ...
        // 4자 이상 사용 제한
		if (keyboardsWord.some(_word => {
			for (let _i = 0; _i < _word.length - 3; _i++) {
				// XXX 역순도 내부에서 처리 가능할 듯?
				if (vPassword.includes(_word.substring(_i, _i + 4))) return true;
			}
