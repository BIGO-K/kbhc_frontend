...
export const useThirdPartyAuth = (platform: ThirdPartyPlatforms) => {
	...
	/** 서드파티 토큰 발급 완료 여부 */
	const checkThirdPartyTokenGenerateStatus = async (platform: ThirdPartyPlatforms, token: string) => {
	...
	const getProfile = async (accessToken: string, platform: string): Promise<ThirdPartyProfile> => {
	...
	return {
		platformLabel,
		/** 컨텍스트 시작 처리 */
		generateEnterContext: async (redirectPathRouter?: RedirectPath) => {
			const context = await useBackend().command<ThirdPartyGenerateEnterContext>(
				`/v1/3rd-party-authentications/${platform}/generate-token`,
				{
					method: 'post',
					params: redirectPathRouter
		...
		/** 컨텍스트 토큰으로 인증결과 조회 */
		getAuthResultByToken: async (): Promise<ThirdPartyProfile> => {
		...
		/** 이메일로 중복회원 조회 */
		getDuplicatesByEmail: async (email: string) => {
			...
			if (duplicate_users.length > 0) expireStore.set('duplicate-social-users', duplicate_users);

			return duplicate_users;
		},
		/** 컨텍스트 토큰으로 소셜 회원가입 요청 */
		registWithToken: async () => {
		...
		nativeCallbackNaverLogin: async (accessToken: string): Promise<{
		...
		setAccessToken: (storeToken: string) => {
			if (!storeToken) {
				throw {
					message: '토큰이 없습니다.',
				};
