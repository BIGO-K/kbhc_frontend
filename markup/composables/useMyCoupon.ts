...
export const useMyCoupon = () => {
	...
	const getApplyRangeDescription = (coupon: RegistedCoupon) => {
		...
		if (coupon.min_sell_price) applyRangeDesc += ` ${formatNumber(coupon.min_sell_price)}원 이상`;
	...
	return {
		...
		/** 쿠폰 사용가능 상품 조회 */
		getUsableGoodsPaginator: async (couponId: number, page: number = 1, pageSize: number = 5) => {
		...
		/** 쿠폰 사용가능 브랜드 조회 */
		getUsableBrandPaginator: async (couponId: number, page: number = 1, pageSize: number = 5) => {
			const { paginator } = await useBackend().command<{
