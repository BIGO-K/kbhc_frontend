...
export const useGoodsCategory = () => {
    ...
    */
     ...
     * @returns
    */
	const find = async (code: string): Promise<CategoryInformation> => {
	...
	return {
		...
		find,
		/** 카테고리에 속한 아이템 갯수 반환 */
		getCategoriesGoodsCount: async (categoryCodes: string[]): Promise<number[]> => {
		...
		activeCategories: (categoryInformation: CategoryInformation | undefined) => {
			// XXX category1,2,3 없이 activeCategories[number]로 사용?
		...
		getExhibitOneDepth: async (categoryCode: string) => {
