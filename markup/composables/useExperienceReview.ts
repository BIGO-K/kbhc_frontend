...
export const useExperienceReview = () => {
	const { isMember } = useGlobalPageContext();

	return {
		/** 체험단 리뷰 페이지네이터 조회 */
		getPaginator: async (page: number = 1, pageSize: number = 20) => {
		...
		/** 체험단 후기 추천여부 확인 */
		getRecommended: async (reviewIds: number[]) => {
			if (!isMember.value) return reviewIds.map(review_id => ({
				review_id,
		...
		/** 체험단 리뷰 작성 */
		add: async (form: ExperienceReviewForm) => {
			const formData = new FormData();
			formData.append('goods_id', `${form.goods_id || ''}`);
		...
		/** 체험단 후기 추천 취소 */
		cancelRecommend: async (id: number) => {
