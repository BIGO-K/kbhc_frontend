...
export const useOrderSheet = (sheet: OrderSheet) => {
	/** 배송 메세지 템플릿 */
	...
	const appliedCouponCountByItemSet = computed(() => {
		...
		for (const set of itemSets.value) {
	...
	/** 최대할인 적용되는 쿠폰 선택값 */
	const maximumDiscountAppliedCouponRegist = computed(() => {
		...
		for (const appliedValue of appliedValues) {
			if (
	...
	const applyMaximumDiscount = computed({
		...
		set: (value) => {
			appliedCouponRegist.value = value === true ? { ...maximumDiscountAppliedCouponRegist.value } : {};
		},
	});
	...
	const discountsByItem = computed<DiscountByItem>(() => {
		...
		orderItems.value.forEach(orderItem => {
			...
			result[orderItem.uuid].nightSaleAmount = orderItem.night_sale_amount ?? 0;
	...
	const priceContext = computed(() => {
		...
		/** 심야할인 */
		const nightSaleAmount = orderItems.value.reduce((sum, orderItem) => sum + (orderItem.night_sale_amount ?? 0), 0);
	...
	return {
		...
		/** 아이템셋의 적용가능 쿠폰발급목록 */
		usableCouponRegists: (itemSet: OrderSheetMergedOrderItems) => {
			const targetItemSet = itemSets.value.find(set => set.key === itemSet.key);
			if (!targetItemSet) return [];
