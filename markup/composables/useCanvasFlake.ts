import { ref, watch, isRef, computed, onBeforeUnmount, MaybeRef } from 'vue';
import { useElementSize } from '@vueuse/core';

// XXX canvas까지 생성하거나 컴포넌트로 변경?
...
export const useCanvasFlake = (canvas: MaybeRef<HTMLCanvasElement | undefined>) => {
	const context = ref<Nullable<CanvasRenderingContext2D>>(null);
	const width = ref(0);
	const height = ref(0);
	const frameId = ref(0);
     ...
     * @returns
    */
	const makeFlake = (fixTop?: boolean): Flake => {
		const flake: Flake = {
        ...
        // x축 잘림 방지 x값 조정
		const maxXY = Math.max(flake.width, flake.height);
		if (flake.x > width.value - maxXY) flake.x = flake.x - maxXY;
		else if (flake.x < maxXY) flake.x = flake.x + maxXY;
