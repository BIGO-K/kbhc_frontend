...
export const useRecentViewGoodsStore = defineStore('recent-view-goods', () => {
	...
	return {
		...
		recordsByRecent,
		/** 최근본상품 로드 */
		load: async (): Promise<RecentViewGoods[]> => {
			...
			records.value = records.value
				.filter(record => !recent_view_goods.some(_item => _item.id !== record.goods.id))
				.concat(
					recent_view_goods.map(_item => ({
						goods: _item,
		 ...
		 */
		add: async (goods: RecentViewGoods) => {
			if (authStore.member) {
				/** 회원은 db처리 */
