<template>
	<div
		ref="$ui"
		:class="`${prefix}_slider`"
	>
		<div
			ref="$inner"
			:class="`${prefix}_slider-inner`"
		>
			<ul
				ref="$list"
				:class="`${prefix}_slider-list`"
			>
				<li
					v-for="(item, _i) in datas"
					:key="`${JSON.stringify(item)}_${_i}_${prop.isLoop && Math.floor(_i / realTotal) === 1 ? 'clone' : ''}`"
					:ref="$el => $items[_i] = $el as HTMLElement"
					:class="[`${prefix}_slider-item`, { [prop.cloneClass]: prop.isLoop && Math.floor(_i / realTotal) === 1 }]"
				...
				>
					...
					<template v-else>
						<div :class="`${prefix}_slider-item-group`">
							<template
		...
		<div
			...
			:class="`${prefix}_slider-pagination`"
		>
			<slot
				name="pagination"
...
<script setup lang="ts" generic="T extends {}">
	...
	const change = (_index: number, _direction?: typeof direction.value, _isDrag?: boolean) => {
		if (_index === activeIndex.value || animation.isAnimated(updateValue)) return;
