...
<script setup lang="ts">
	...
	interface LazyloadProp {
		/** @prop { string | null } `default: null` - 이미지 경로 */
		src?: Nullable<string>;
		/** @prop { string | null } `default: null` - src가 없을 때 추가로 불러올 이미지 경로 */
		src2?: Nullable<string>;
		/** @prop `default: i` - 생성할 태그로 i는 background로 적용 */
		tag?: 'i' | 'img';
		/** @prop { string | null } `default: null` - tag가 img 일 때 적용되는 alt로 error 대응을 위해 prop에 추가 */
		alt?: Nullable<string>;
	...
	const errorParentClass = computed(() => !prop.isErrorImage || !isError.value ? undefined : `${prop.isBlockContent ? 'mui' : 'mm'}_image-none`);
	const errorIconClass = computed(() => !prop.isErrorImage || !isError.value ? undefined : `${prop.isBlockContent ? 'uico' : 'ico'}_none`);
	const stateClass = ref<Nullable<typeof loadingClass | typeof completeClass | typeof errorClass>>(null);
	const ratioClass = ref<string>();
	const dataIr = ref<Nullable<string>>();
