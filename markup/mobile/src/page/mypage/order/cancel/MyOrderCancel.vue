								...
								<div class="mm_product-item">
									<mm-link :to="{ name: 'ProductDetail', params: { id: orderItem.id } }">
										<figure>
											...
											<figcaption>
												<p class="text_status">
													{{ orderItem.order_status_label }}<b class="mm_tag T=bg T=gray">{{ orderItem.refund_status_label }}</b>
												</p>
												<p class="text_brand">
...
<script setup lang="ts">
	...
	const sortRange = ref('');
	const cancelOrdersData: CancelStepOrder[] = [
		{
					...
					'sellers': [
						{
							...
							'order_items': [
								{
									'id': 374,
									'order_status_label': '취소 요청',
									'refund_status_label': '환불 요청',
									'paid_price': 41800,
									'goods': {
										'id': 2,
										'name': '드시온 귀리렌틸콩 곤약밥',
	...
	const cancelOrders = ref<CancelStepOrder[]>(cancelOrdersData);
</script>