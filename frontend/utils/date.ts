 ...
 * ```
 */
const monsDate = (date?: DateInput | Dayjs): DateHelper => {
	...
	return {
		...
		format: (format: string, isKo: boolean = false): string => {
			if (isKo) return targetDate.locale('ko').format(format);
			else return targetDate.format(format);
		},
		isValid: (): boolean => {
		...
		/** 날짜가 연도와 월이 더해져서 계산 되기 때문에 연도와, 달을 사용하는 경우 추가 작업이 필요 */
		duration: (compare: DateInput, unit?: OpUnitType): Duration => {
			const durationResult = dayjs.duration(targetDate.diff(compare, unit));
