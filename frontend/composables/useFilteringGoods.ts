...
export const useFilteringGoods = (
	totalCount: number,
	initFilterable: GoodsFilterable,
	initGoodsList: Nullable<Goods>[][],
	...
	const goodsListByPage = ref<Nullable<Goods>[][]>(initGoodsList ?? []);
	...
	watch(sorting, (newSorting, oldSorting) => {
		if (newSorting === oldSorting) return;
	...
	return {
		...
		/** `선택상태`를 컨텍스트에 반영 */
		applySelected: (newSelected?: GoodsFilterSelected) => {
			// selected -> paramsForCommand
			paramsForCommand.value.page = 1;
			paginator.value.current_page = 1;
			if (newSelected) selected.value = newSelected;
		...
		toggleChildrenCategory: (parentCategory?: CategoryForFilter) => {
			...
			const childrenCodes = parentCategory.child_categories.map(c => c.code);
			const filteredCategories = selected.value.categories.filter(selectedCategory => !childrenCodes.includes(selectedCategory.code));

			selected.value.categories = isChildrenCategoryAllSelected(parentCategory)
				? filteredCategories
