...
export const useMyShopping = () => {
	return {
		/** 주문 현황 조회 */
		getOrderStateContext: async () => {
		...
		/** 최근 본 기획전 목록 조회 */
		getRecentViewPlanningEvents: async (limit: number = 30) => {
		...
		/** 포인트내역 조회 */
		getPointHistories: async (from: string, to: string, page: number = 1, pageSize: number = 20) => {
		...
		getGradeUpCondition: async () => {
			const { grade_up_condition } = await useBackend().command<{
				grade_up_condition: MemberGradeUpCondition;
			}>('v1/my/grade-up-condition');

			return {
				condition: grade_up_condition,
				progress: {
					order_amount: grade_up_condition.order_amount / grade_up_condition.grade_up_order_amount_condition * 100,
					order_count: grade_up_condition.order_count / grade_up_condition.grade_up_order_count_condition * 100,
				},
				requirement: {
					order_amount: Math.max(grade_up_condition.grade_up_order_amount_condition - grade_up_condition.order_amount, 0),
					order_count: Math.max(grade_up_condition.grade_up_order_count_condition - grade_up_condition.order_count, 0),
				},
			};
		},
	};
