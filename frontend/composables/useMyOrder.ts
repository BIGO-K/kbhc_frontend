...
type ToOrderItemSetMappable = Optional<OrderItem, 'goods' | 'paid_price' | 'star_point_used' | 'reward_point_used'>;
...
export const useMyOrder = () => {
	...
	const CLAIM_REASON_ICON_CLASS: Record<ClaimReasonIconLabel, ClaimReasonIconClassName> = {
		...
		wrong_goods_delivered: 'delivery',
	};
	...
	const defaultRefundInfo: RefundInfo = {
		...
		refund_price: 0,
		refund_star_point: 0,
		refund_reward_point: 0,
		calculate_details: [],
	};

	const defaultExchangeAdditionalPayments: ExchangePaymentInfo = {
		total_price: 0,
		calculate_details: [
			{
				label: '일반 배송비',
	...
	/** 주문상품 묶음처리 */
	const orderItemToSet = <T extends ToOrderItemSetMappable>(orderItems: T[]): OrderItemSet<T>[] => {
		...
		orderItems.forEach(item => {
			...
			if (!orderItemSets[targetKey]) {
				orderItemSets[targetKey] = {
					...
					total_star_point_used: item.star_point_used,
					total_reward_point_used: item.reward_point_used,
					items: [deepClonedOrderItem],
				};
			}
			else {
				orderItemSets[targetKey].items.push(deepClonedOrderItem);
				orderItemSets[targetKey].total_paid_price += item.paid_price;
				orderItemSets[targetKey].total_star_point_used += item.star_point_used;
				orderItemSets[targetKey].total_reward_point_used += item.reward_point_used;
	...
	return {
		...
		/** [정상] 단계 주문 목록 조회 */
		getNormalStepOrders: async (monthValue: number, statusCode?: Nullable<number>) => {
		...
		/** [정상] 구매확정처리 */
		confirmPurchase: async (orderItemId: number) => {
		...
		/** [정상] 수취 확인 처리 */
		confirmReceipt: async (orderItemId: number) => {
		...
		updateShippingAddressId: async (orderId: string, shippingAddressId: number) => {
		...
		updateShippingAddress: async (orderId: string, form: MyShippingAddress) => {
		...
		/** [취소] 단계 주문 목록 조회 */
		getCancelStepOrders: async (monthValue: number) => {
			const targetMonth = monthTabs.find(month => month.value === monthValue) || monthTabs[0];
		...
		/** [취소] 주문 상세 */
		getCancelDetail: async<T extends boolean>(cancelId: string, isSet?: T) => {
			...
			if (isSet) {
				cancel_detail.packs = cancel_detail.packs.map(pack => {
					pack.order_items = orderItemToSet<CancelDetailOrderItem>(pack.order_items);

					return pack;
				});
			}

			return cancel_detail as T extends true ? CancelOrderDetailSet : CancelOrderDetail;
		},
		/** [취소] 가능 주문 조회 */
		getCancelables: async (orderId: string, orderItemId: number) => {
			...
			} as CancelableOrderSet;

			return cancelOrderItemSet;
		},
		/** [취소] 환불금액 정보 조회 */
		getCancelRefundInfo: async (orderId: string, orderItemIds: number[], reasonCode: number) => {
		...
		/** [취소] 신청 */
		applyCancel: async (form: CancelForm) => {
		...
		/** [반품] 단계 주문 목록 조회 */
		getReturnStepOrders: async (monthValue: number) => {
			const targetMonth = monthTabs.find(month => month.value === monthValue) || monthTabs[0];
		...
		/** [반품] 상세 조회 */
		getReturnDetail: async <T extends boolean>(returnId: string, isSet?: T) => {
			...
			if (isSet) return_detail.pack.order_items = orderItemToSet(return_detail.pack.order_items);

			return return_detail as T extends true ? ReturnOrderDetailSet : ReturnOrderDetail;
		},
		/** [반품] 가능 주문 조회 */
		getReturnable: async (orderId: string, orderItemId: number) => {
		...
		/** [반품] 환불금액 정보 조회 */
		getReturnRefundInfo: async (orderId: string, form: ReturnRefundPredictForm) => {
			const { refund_info } = await useBackend().command<{
				refund_info: RefundInfo;
		...
		/** [반품] 신청 */
		applyReturn: async (form: ReturnForm) => {
		...
		/** [반품] 철회 신청 */
		cancelReturn: async (returnId: string, returnTargetId: number) => {
		...
		/** [반품] 송장정보 수정 */
		updateReturnRetrieveInvoice: async (form: ReturnRetrieveInvoiceUpdateForm) => {
		...
		/** [교환] 단계 주문 목록 조회 */
		getExchangeStepOrders: async (monthValue: number) => {
			const targetMonth = monthTabs.find(month => month.value === monthValue) || monthTabs[0];
		...
		/** [교환] 주문 상세 */
		getExchangeDetail: async <T extends boolean>(exchangeId: string, isSet?: T) => {
			...
			exchange_detail.retrieve_address = exchange_detail.retrieve_address?.base_address
				? exchange_detail.retrieve_address
				: null;

			if (isSet) exchange_detail.pack.order_items = orderItemToSet(exchange_detail.pack.order_items);

			return exchange_detail as T extends true ? ExchangeOrderDetailSet : ExchangeOrderDetail;
		},
		/** [교환] 가능 주문 조회 */
		getExchangeable: async (orderId: string, orderItemId: number) => {
		...
		/** [교환] 추가결제정보 조회 */
		getExchangePaymentInfo: async (orderId: string, form: ExchangePaymentPredictForm) => {
		...
		/** [교환] 신청 */
		applyExchange: async (form: ExchangeForm) => {
		...
		prepareExchangePay: async (form: ExchangePaymentForm) => {
		...
		/** [교환] 추가결제 결과 조회 */
		getExchangePayResult: async (paymentToken: string) => {
		...
		/** [교환] 철회 신청 */
		cancelExchange: async (exchangeId: string, exchangeTargetId: number) => {
		...
		getExchangeToReturnable: async (exchangeId: string) => {
			...
			} as ExchangeToReturnableOrderSet;
		...
		/** [교환] 반품전환 환불금액정보 조회 */
		getExchangeToReturnRefundInfo: async (exchangeId: string, exchangeTargetIds: number[]) => {
			const { refund_info } = await useBackend().command<{
				refund_info: RefundInfo;
		...
		/** [교환] 반품전환 신청 */
		applyExchangeToReturn: async (form: ExchangeToReturnForm) => {
		...
		/** [교환] 송장정보 수정 */
		updateExchangeRetrieveInvoice: async (form: ExchangeRetrieveInvoiceUpdateForm) => {
		...
		getPaymentReceipt: async (orderId: string) => {
			...
			receipt.cancels = receipt.cancels.map(cancel => {
				cancel.type = 'cancel';
		...
		/** 영수증 이메일로 전송 */
		sendPaymentReceiptToEmail: async (
			orderId: string,
			email: string,
		...
		/** 송장 유효성 검사 */
		validateInvoice: async (form: RetrieveInvoiceForm) => {
		...
		getRefundCalculations: (refundInfo: RefundInfo) => {
			// 환불상세 합산 값
			const sumOfCalculateDetail = refundInfo.calculate_details.reduce((accumulator, current) => {
				return accumulator + (current.is_substracted ? -Number(current.amount) : Number(current.amount));
			}, 0);
		...
		addCart: async (orderItemIds: number[]) => {
			await useBackend().command('v1/shopping/carts/order-items', {
				method: 'post',
				data: {
					order_item_ids: orderItemIds,
				},
