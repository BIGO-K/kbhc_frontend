...
import { STORE_BASE_KEY } from '$/constants';
...
pinia
	.use((context) => {
		...
		context.store.$reset = () => {
			...
			for (const persist of context.options.persist) {
				(persist.storage as Storage).removeItem(context.store.$id);
			}
		};
	})
	.use(createPersistedState({
        // 커스텀 키 적용
		key: (key) => `${STORE_BASE_KEY}${key}`,
	}));
