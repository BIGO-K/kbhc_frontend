...
/** 주문내역 */
type MyOrder<OrderItemType> = (OrderItemType extends NormalStepOrderItem ? {
	bundles: OrderBundlePack<OrderItemType>[];
} : {
...
type OrderDetail = {
	...
	order: {
		...
		bundles: OrderBundlePack<OptionalNullable<OrderItem, OrderItemNullableKey>>[];
	};
};

/** [custom] 주문 묶음단위 (포장단위보다 상위) */
type OrderBundlePack<OrderItemType> = {
	bundle_goods_name: Nullable<string>;
	bundle_goods_id: Nullable<string>;
	bundle_goods_no: Nullable<number>;
	packs: OrderSellerPack<OrderItemType>[];
};
...
/** 주문 포장 단위(셀러) */
type OrderSellerPack<OrderItemType> = {
	/** 배송비 */
	paid_shipping_price: number;
	/** 배송정책 ID */
	shipping_rule_id: number;
...
type OrderItem = {
	...
	/** 반송조회 url */
	return_delivery_tracking_url: Nullable<string>;
	/** [교환주문] 반품전환 가능 여부 */
	...
	/** [custom:무형상품] 사용여부 */
	is_intangible_product_used: boolean;
	/** [custom:무형상품] 사용종료여부 */
	is_intangible_product_expired: boolean;
	/** 상품 */
	goods: OrderItemGoods;
};

type OrderItemRequiredKey = Extract<keyof OrderItem, 'id' | 'goods'>;
type OrderItemNullableKey = Extract<
Exclude<keyof OrderItem, OrderItemRequiredKey>,
'refund_status_label' |
'cancel_id' |
'return_id' |
'return_target_id' |
...
type OptionalOrderItem = Optional<OptionalNullable<OrderItem, OrderItemNullableKey>, OrderItemRequiredKey>;

type OrderItemGoods = Pick<Goods,
'id' |
'name' |
'brand_name' |
'thumbnail_url'
...
type OrderItemSet<T extends OptionalOrderItem<'goods' | 'paid_price' | 'star_point_used' | 'reward_point_used'>> = T & {
	goods: T['goods'] & {
		ea: number;
	};
	total_paid_price: T['paid_price'];
	// total_point_used: number;
	total_star_point_used: T['star_point_used'];
	total_reward_point_used: T['reward_point_used'];
	items: T[];
};
...
/** [정상] 주문상품 */
type NormalStepOrderItem = Pick<OrderItem,
'id' |
'order_status_label' |
'paid_price' |
'earnable_point' |
...
'returnable' |
'exchangeable' |
'is_intangible_product_expired' |
'is_intangible_product_used' |
'voucher_code' |
'goods'
>;
...
/** [취소] 주문상품 */
type CancelStepOrderItem = Pick<OrderItem,
'id' |
'order_status_label' |
'paid_price' |
'star_point_used' |
...
/** [취소] 주문 상세 주문상품 */
type CancelDetailOrderItem = Pick<OrderItem, 'id' | 'paid_price' | 'star_point_used' | 'reward_point_used'> & {
	seller_name: string;
	/** 상품 */
	goods: OrderItemGoods & {
		is_soldout: boolean;
...
/** [취소] 가능 주문 베이스 */
type BaseCancelableOrder<CancelOrderItem> = Pick<ClaimableOrder<CancelOrderItem>,
'order_id' |
'ordered_at' |
'packs' |
...
/** [취소] 가능 주문상품 */
type CancelableOrderItem = Pick<OrderItem,
'id' |
'order_status_label' |
'paid_price' |
'star_point_used' |
...
type ClaimReasonIconClassName = 'change' | 'defects' | 'delivery';
...
/** [반품] 주문상품 */
type ReturnStepOrderItem = Pick<OrderItem,
'id' |
'return_target_id' |
'order_status_label' |
'paid_price' |
...
type BaseReturnOrderDetail<ReturnOrderItem> = {
	...
	/** 반품 수거지 */
	retrieve_address: Nullable<MyShippingAddress>;
	/** 환불계좌 */
	refund_account: Nullable<RefundAccount>;
	/** 환불정보 */
	refund_info: RefundInfo;
...
/** [반품] 주문상세 주문상품 */
type ReturnOrderDetailItem = Pick<OrderItem, 'id' | 'paid_price' | 'star_point_used' | 'reward_point_used'> & {
	seller_name: string;
	/** 상품 */
	goods: OrderItemGoods & {
		is_soldout: boolean;
...
/** [반품] 가능 주문 베이스 */
type BaseReturnableOrder<ReturnOrderItem> = Pick<ClaimableOrder<ReturnOrderItem>,
'order_id' |
'ordered_at' |
'packs' |
'payment_method_label' |
...
/** [반품] 가능 주문상품 */
type ReturnableOrderItem = Pick<OrderItem,
'id' |
'order_status_label' |
'paid_price' |
'star_point_used' |
...
/** [교환] 주문상품 */
type ExchangeStepOrderItem = Pick<OrderItem,
'id' |
'order_status_label' |
'exchange_target_id' |
'exchanged_option_name' |
...
/** [교환] 가능 주문 베이스 */
type BaseExchangeableOrder<ExchangeOrderItem> = Pick<ClaimableOrder<ExchangeOrderItem>,
'order_id' |
'ordered_at' |
'packs' |
'payment_method_label' |
...
/** [교환] 주문 상세 주문상품 */
type ExchangeOrderDetailItem = Pick<OrderItem,
'id' |
'paid_price' |
'star_point_used' |
'reward_point_used' |
...
/** [교환] 반품전환 가능 주문 */
type BaseExchangeToReturnableOrder<OrderItemType> = Pick<ClaimableOrder<OrderItemType>,
'order_id' |
'ordered_at' |
'packs' |
'payment_method_label' |
...
/** [교환] 반품전환 가능 주문상품 */
type ExchangeToReturnableOrderItem = Pick<OrderItem,
'id' |
'exchange_target_id' |
'order_status_label' |
'paid_price' |
...
type RefundInfo = {
	...
	// refund_point: number;
	/** [custom] 환불 스타포인트 */
	refund_star_point: number;
	/** [custom] 환불 리워드포인트 */
	refund_reward_point: number;
	/**	차감/합산 상세 내역 리스트 */
	calculate_details: {
		/** 계산내역 라벨 */
		label: string;
