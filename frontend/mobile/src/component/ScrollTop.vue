<template>
	...
	<a
		...
		@click.prevent="_e => overlayProductRecently(_e)"
	>
		<lazyload
			:key="recentViewGoodsImage || ''"
...
<script setup lang="ts">
	...
	/** 최근 본 상품 */
	const overlayProductRecently = (_e: MouseEvent) => {
		openOverlay(defineAsyncComponent(() => import('@/page/product/recently/ProductRecently.vue')), {
			props: {
	...
	const needHeaderSticky = computed(() => pageContextStore.headerSticky);
	...
	watch(y, (_currentScrollY, _beforeScrollY) => {
		...
		if (needHeaderSticky.value) return;
		// 헤더 노출/숨김
		if ($header.value) {
