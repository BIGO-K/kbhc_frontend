...
<script setup lang="ts">
	...
	const props = withDefaults(defineProps<{
		...
		totalCount?: number;
		/** 세로 스크롤 타겟 위치 */
		scrollYTarget?: Nullable<HTMLElement>;
		/** 라우트 변경처리 여부 */
	...
	/** 마지막 페이지 번호 */
	const lastPage = computed(() => Math.ceil(props.totalCount / props.itemsPerPage));
	/** 이전 페이지블록의 마지막 페이지 번호 */
	const lastPageOfPreviousBlock = computed<number>(() => Math.max(currentPageItems.value[0] - 1, 1));
	/** 다음 페이지블록의 첫번째 페이지번호 */
	const firstPageOfNextBlock = computed<number>(() => Math.min(currentPageItems.value[props.pageBlockCount - 1] + 1, lastPage.value));

	/** 현재 페이지블록 */
	const currentPageBlock = computed(() => Math.ceil(currentPage.value / props.pageBlockCount));
	const lastPageBlock = computed(() => Math.ceil(lastPage.value / props.pageBlockCount));

	/** 현재 페이지아이템 리스트 */
	const currentPageItems = computed<number[]>(() => {
		return Array(...Array(props.pageBlockCount))
			.map((_, index) => (currentPageBlock.value - 1) * props.pageBlockCount + (index + 1))
	...
	function scrollTop() {
		if (!props.scrollYTarget) {
			return;
		}

		props.scrollYTarget.scrollIntoView();
	}
</script>