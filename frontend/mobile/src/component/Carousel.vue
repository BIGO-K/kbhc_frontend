<template>
	<div
		ref="$ui"
		:class="`${prefix}_carousel`"
	>
		<div
			ref="$inner"
			:class="`${prefix}_carousel-inner`"
		>
			<ul
				ref="$list"
				:class="`${prefix}_carousel-list`"
			>
				<li
					v-for="(item, _i) in datas"
					:key="`${JSON.stringify(item)}_${_i}_${useClone && _i >= realTotal ? 'clone' : ''}`"
					:ref="$el => $items[_i] = $el as HTMLElement"
					:class="[`${prefix}_carousel-item`, { [prop.cloneClass]: useClone && _i >= realTotal }]"
				...
				>
					...
					<template v-else>
						<div :class="`${prefix}_carousel-item-group`">
							<template
		...
		<div
			...
			:class="`${prefix}_carousel-pagination`"
		>
			<slot
				name="pagination"
				...
				<ol>
					...
					>
						<button
							...
							@click="change(_i - 1, _i > activeIndex ? 'next' : 'prev')"
						>
		...
		<div
			...
			:class="`${prefix}_carousel-control`"
		>
			<button
				ref="$btnPrev"
				type="button"
...
<script setup lang="ts" generic="T extends {}">
	...
	const prop = withDefaults(defineProps<{
		...
		controlIconClass?: Nullable<string>;
	...
	const datas = computed(() => {
		const returnDatas = (prop.groupSize < 2)
			? prop.items
			: chunk(prop.items, prop.groupSize);
	...
	const isDirection = ref<Nullable<boolean>>(null);
	...
	const change = (_index: number, _direction?: typeof direction.value, _isBack?: boolean) => {
		if (_index === activeIndex.value || animation.isAnimated(updateValue)) return;// _index가 같거나 모션 중에는 리턴
	...
	// 드래그
	useEventListener($inner, isMobileUser ? 'touchstart' : 'mousedown', _e => {
		if (realTotal.value < 2) return;// 1개일 때 사용안함

		isDirection.value = null;
		if (animation.isAnimated(updateValue)) return;// 모션 중에는 리턴
		...
		const touchEventOff = useEventListener(document, isMobileUser ? ['touchmove', 'touchend'] : ['mousemove', 'mouseup'], _e => {
			if (_e instanceof MouseEvent) _e.preventDefault();
	...
	useEventListener($ui, isMobileUser ? ['touchstart', 'touchend'] : ['mouseover', 'mouseleave'], _e => {
		if (autoDelay.value <= 0 || realTotal.value < 2) return;
