...
<script setup lang="ts">
	...
	const closer = <OverlayCloseContext>(context?: OverlayCloseContext) => {
		if (layer.value.onClose) layer.value.onClose(context);

		layoutStore.removeLayer(layer.value.uuid);
	};

	const close = () => {
		if (layer.value.onCancelClose) layer.value.onCancelClose();
