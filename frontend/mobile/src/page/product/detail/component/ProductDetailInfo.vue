<template>
	...
	>
		<div class="mm_inner">
			...
			<p class="text_code">
				상품번호<span>{{ goodsProvider }} {{ goodsId }}</span>
			</p>
...
<script setup lang="ts">
	...
	const isPinchZoomActive = ref(false);

	const toggleDetailOpen = (_isActive: boolean) => {
		useNotice.value = _isActive;
		isPinchZoomActive.value = _isActive;
	...
	const pinchZoomData = ref({
		...
		scaleMax: 2, // ? :number - 핀치 줌인 가능한 최대 배수
	...
	function touchStartEventHandler(event: TouchEvent) {
		...
		zoom.value.beforeScale = zoom.value.scale = Number(animation.getProperty($pinchzoomContent.value, 'scaleX'));

		translate.beforeX = Number(animation.getProperty($pinchzoomContent.value, 'x'));
		translate.beforeY = Number(animation.getProperty($pinchzoomContent.value, 'y'));
	...
	function touchMoveEventHandler(event: TouchEvent) {
		...
		if (isOnDrag || isOnZoom) {
			...
			animation.set($pinchzoomContent.value, {
	...
	function touchEndEventHandler(event: TouchEvent) {
		...
		if (zoom.value.scale <= 1) {
			...
			const changeScroll = $scroller.value.scrollTop - Number(animation.getProperty($pinchzoomContent.value, 'y'));
			$scroller.value.scrollTop = changeScroll;
