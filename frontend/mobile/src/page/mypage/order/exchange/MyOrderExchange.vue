...
<script setup lang="ts">
	...
	const { monthTabs, getExchangeStepOrders } = useMyOrder();
	...
	async function getExchangeOrders() {
		loading.show();
		try {
			exchangeOrders.value = await getExchangeStepOrders(filter.value.month);
			router.replace({
				path: route.path,
				query: {
	...
	await getExchangeOrders();
	watch(filter.value, getExchangeOrders);
</script>