	...
	<div class="m_receipt">
		<div class="m_receipt-head">
			...
			>
				...
				<template v-if="receipt.cancels.length > 1">
					<option
						v-for="(cancelDetail, _i) in receipt?.cancels"
						:key="cancelDetail.paid_at"
						:value="`cancel_${_i}`"
					>
						부분취소({{ _i + 1 }})
					</option>
		...
		>
			<section>
				...
				<table>
					<colgroup>
						<col :style="`width: ${tableHeadWidth}px;`">
						<col>
					</colgroup>
					<tbody>
						<tr>
...
<script setup lang="ts">
	...
	const tableHeadWidth = 121;
	const maskedOrdererName = prop.ordererName.replace(/.(?=.$)/u, '*');

	const shopInfo = ref(config('shop'));

	const receipt = await getPaymentReceipt(prop.orderId);
	...
	const bomReceiptEmail = () => {
		bom.prompt(
			...
			async (_is: boolean, _values: Record<string, string>) => {
				...
				try {
					await sendPaymentReceiptToEmail(prop.orderId, _values.email, type, refundId);
			...
			},
			{
				title: '메일 발송',
				forms: [
					{ type: 'text', name: 'email', maxLength: 50, placeholder: `‘@’을 포함하여 50자 이하 입력`, value: '' },
				],
			}
		);
	};
