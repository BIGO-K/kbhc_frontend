			...
			<ul>
				<li>
					...
					>
						...
						<template #content>
							<template
								v-for="goods in bundleGoods"
								:key="goods"
							>
								<h5><b>{{ goods.name }}</b></h5>
								<table>
									<tbody>
										<tr>
											<th scope="row">
											...
											<td>{{ goods.delivery_info?.shipping_center_name ?? '-' }}</td>
										</tr>
										<!--
											<tr>
											<th scope="row">
										...
										<tr>
											...
											<td>
												{{ goods.delivery_info?.today_exportable_message === '시 이전 결제 시 당일 출고됩니다.' ? '-' : goods.delivery_info?.today_exportable_message }}
											</td>
										</tr>
									</tbody>
								</table>
								...
								<table>
									<tbody>
										<tr>
											...
											<td>{{ goods.return_delivery_info?.company_name ?? '-' }}</td>
										</tr>
										<tr>
											<th scope="row">
												<b>연락처</b>
											</th>
											<td>{{ goods.return_delivery_info?.tel ?? '-' }}</td>
										</tr>
										<tr>
											<th scope="row">
												<b>택배사</b>
											</th>
											<td>{{ goods.return_delivery_info?.shipping_company ?? '-' }}</td>
										</tr>
										<tr>
											<th scope="row">
												<b>반품 배송비</b>
											...
											<td>
												<template v-if="!goods.delivery_info && !goods.return_delivery_info">
													-
												</template>
												<template v-else>
													구매자 귀책: {{ number((goods.delivery_info?.shipping_price || 0) + (goods.return_delivery_info?.return_shipping_price || 0) ) }}원
													또는 {{ number(goods.return_delivery_info?.return_shipping_price || 0) }}원
												</template>
											</td>
										</tr>
										<tr>
											...
											<td>{{ goods.return_delivery_info?.return_address || '-' }}</td>
										</tr>
									</tbody>
								</table>
							</template>
...
<script setup lang="ts">
	...
	const props = defineProps<{
		bundleChildeGoods: BundleChildGoods[];
	}>();

	const bundleGoods = computed(() => props.bundleChildeGoods);
	...
	// 		return '무료배송';
	// 	}
	// 	const conditionFreeMessage = delivery_info?.is_conditional_free_delivery ? `\n ${number(delivery_info.conditional_free_price)}원 이상 구매 시, 무료 배송` : '';
	// 	const packageMessage = delivery_info?.is_package_delivery ? '' : '(개별 부과)';

	// 	return `배송비 ${number(delivery_info?.shipping_price)} 원 ${packageMessage} ${conditionFreeMessage}`;
	// });
</script>